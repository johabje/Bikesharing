...
def getAreas(month, config):
    ...
    stations=os.listdir(f"Data/Dataset_clusters/{config}/with_avail/{month[-1]}")
...
def Test_train_month(pred_periods, months, config):
    """Implements the random prediction model for the given number of periods"""

    areas = getAreas(months, config)
    models = dict()
    testX = dict()
    testY = dict()
    CO_pred = dict()
    ...
    #create output json file and folder if not existent
    if not os.path.exists(f'Clustering/results/{config}/svm'):
        os.makedirs(f'Clustering/results/{config}/svm')

    with open(f'Clustering/results/{config}/svm/CO_RF_pred.json', 'w') as fp:
        json.dump(CO_pred, fp)
    with open(f'Clustering/results/{config}/svm/testY.json', 'w') as fp:
        json.dump(testY, fp)
