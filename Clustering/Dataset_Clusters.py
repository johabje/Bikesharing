...
def main(tripdata, clusters, startTime, endTime):
    ...
    #save counts to file
    for station in counts:
        counts[station][0].to_csv(f'Dataset_Clusters/06/{station}_{startTime.year}_{startTime.month}.csv', index=True)
...
tripdata = pd.read_csv("tripdata/2022/06.csv", parse_dates=True)
